...
with st.sidebar:
    ...
    st.write("This option utilizes statistical techniques like ARIMA (AutoRegressive Integrated Moving Average) and Holt-Winters Exponential Smoothing. ARIMA is suitable for capturing linear relationships and seasonality in time-series data, while Holt-Winters is effective for seasonal and trend data.")
    
    st.write("🤖 **Machine Learning Models Forecasting:**")
    st.write("Machine learning models, such as Random Forest and MLP Regressor, are employed for forecasting. Random Forest is an ensemble learning method that uses multiple decision trees to make predictions, suitable for capturing complex relationships in data. MLP Regressor, or Multi-Layer Perceptron Regressor, is a type of artificial neural network capable of learning non-linear patterns in data.")
